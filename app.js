...
function nowMelbMinutes() {
  const s = new Date().toLocaleTimeString("en-AU", {
    timeZone: TZ,
    hour: "2-digit",
    minute: "2-digit",
    hour12: false,
...
async function loadWeather() {
  ...
  try {
    ...
    const sunLabel = isDark
      ? nowM < riseM
        ? `Sunrise ${fmtIso(day.sunrise[0])}`
        : `Sunrise ${fmtIso(day.sunrise[1] ?? day.sunrise[0])}`
    ...
    const rfText = escHtml(
      realFeelText(Math.round(c.temperature_2m), apparentRounded, windRounded),
    );
...
async function loadAirQuality() {
  ...
  try {
    ...
    const pollen =
      cur.grass_pollen != null ? Math.round(cur.grass_pollen) : null;
...
const VICROADS_URL =
  "https://traffic.transport.vic.gov.au/places?id=";
...
function loadCommunityFeeds() {
  ...
  const eventsInner =
    ...
    staticLink(
      BASE_MERRIBEK + "/exploring-merri-bek/events/whats-on/",
      "Merri-bek What's On",
      "merri-bek.vic.gov.au",
    ) +
    staticLink(
      "https://www.eventbrite.com.au/d/australia--brunswick/events/",
      "Eventbrite · Brunswick",
  ...
  const newsInner =
    `<div class="community-links">` +
    staticLink(
      BASE_MERRIBEK + "/my-council/news-and-publications/news/",
      "Council News",
      "merri-bek.vic.gov.au",
    ) +
    staticLink(
      "https://conversations.merri-bek.vic.gov.au/",
      "Open Projects",
      "Have your say",
    ) +
    `</div>`;
  ...
  const servicesInner =
    ...
    staticLink(
      BASE_MERRIBEK +
        "/living-in-merri-bek/waste-and-recycling/bins-and-collection-services/hard-waste-collection/",
      "Hard Waste Collection",
      "Free collection",
    ) +
    `</div>`;

  const utilitiesInner =
    `<div class="community-links">` +
    staticLink(
      "https://www.citipower.com.au/power-outages-and-emergencies/live-outage-map/?postcode=3056",
      "CitiPower Outages",
      "Electricity (Brunswick area)",
    ) +
    staticLink(
      "https://faults.yvw.com.au/",
      "Yarra Valley Water",
      "Outages & works",
    ) +
    `</div>`;
  ...
  el.innerHTML =
    ...
    section(
      ...
      staticLink(
        VICROADS_URL,
        "VicRoads Live Traffic",
        "Disruptions &amp; roadworks",
      ),
    ) +
    `</div>`;
}
...
for (const [g, list] of Object.entries(AMENITY_GROUPS))
  for (const a of list) AMENITY_TO_GROUP[a] = g;
...
const SHOP_TO_GROUP = {
  ...
  art: "Shops",
  // Health
  chemist: "Health & Wellbeing",
  optician: "Health & Wellbeing",
  hairdresser: "Health & Wellbeing",
  beauty: "Health & Wellbeing",
  massage: "Health & Wellbeing",
  tattoo: "Health & Wellbeing",
  gym: "Health & Wellbeing",
  yoga: "Health & Wellbeing",
  laundry: "Health & Wellbeing",
  dry_cleaning: "Health & Wellbeing",
  hearing_aids: "Health & Wellbeing",
  sauna: "Health & Wellbeing",
  // Services
  travel_agency: "Money & Services",
  estate_agent: "Money & Services",
  tailor: "Money & Services",
  cobbler: "Money & Services",
  locksmith: "Money & Services",
  printing: "Money & Services",
  insurance: "Money & Services",
  key: "Money & Services",
  // Arts
...
function openStatus(oh) {
  ...
  for (const rule of s
    ...
    if (!m) continue;
    const [, ds, tf, tt] = m;
    const from = parseMin(tf),
      to = parseMin(tt);
    let dayMatch = false;
    if (ds.includes(","))
      dayMatch = ds.split(",").some((d) => DAY_IDX[d.trim()] === today);
    else if (ds.includes("-")) {
      const [a, b] = ds.split("-"),
        ai = DAY_IDX[a],
        bi = DAY_IDX[b];
      if (ai != null && bi != null)
        dayMatch =
          ai <= bi ? today >= ai && today <= bi : today >= ai || today <= bi;
    } else dayMatch = DAY_IDX[ds] === today;
    if (!dayMatch) continue;
    const inTime =
      from < to ? nowM >= from && nowM < to : nowM >= from || nowM < to;
    if (!inTime) continue;
    const minsLeft = from < to ? to - nowM : to + 1440 - nowM;
    const d = new Date();
    d.setHours(Math.floor(to / 60), to % 60, 0);
    const closesAt = d.toLocaleTimeString("en-AU", {
      timeZone: TZ,
      hour: "numeric",
      minute: to % 60 ? "2-digit" : undefined,
    });
...
function haversine(la1, lo1, la2, lo2) {
  ...
  const a =
    ...
    Math.cos(f1) * Math.cos(f2) * Math.sin(((lo2 - lo1) * p) / 2) ** 2;
...
const KEYWORDS = [
  ...
  {
    label: "Gym / Yoga",
    amenities: ["gym", "fitness_centre", "yoga", "sports_centre"],
  },
...
function renderPlaces() {
  ...
  const filtered = _places.filter((p) => {
    ...
    if (_activeKeyword && !_activeKeyword.amenities.includes(p.amenity))
      return false;
    if (!term) return true;
    return (
  ...
  const distStr =
    maxDist >= 1000 ? (maxDist / 1000).toFixed(1) + "km" : maxDist + "m";
  const filterStr = [
    _activeKeyword ? escHtml(_activeKeyword.label) : "",
    term ? `"${escHtml(term)}"` : "",
  ]
    .filter(Boolean)
    .join(" · ");
  ...
  for (const group of Object.keys(AMENITY_GROUPS)) {
    ...
    for (const p of list) {
      ...
      const metaTxt =
        p.status.open === true && p.status.closesAt
...
async function loadPlaces() {
  ...
  const amenityList = [
    ...new Set([
      ...Object.values(AMENITY_GROUPS).flat(),
      ...Object.keys(SHOP_TO_GROUP),
    ]),
  ].join("|");
        ...
        return [
          {
            ...
            lat,
            lon,
            dist: haversine(LAT, LNG, lat, lon),
            website,
