...
const AMENITY_GROUPS = {
  'Eat & Drink': [
    ...
    'wine_bar','food_court','juice_bar','deli','biergarten',
  ],
  'Shops': [
    'supermarket','convenience','greengrocer','butcher','deli','newsagent',
    'florist','books','music','clothing','shoes','bicycle','hardware',
    'gifts','pet','stationery','art','alcohol','wine','chemist','optician',
  ],
  'Health & Wellbeing': [
  ...
  'Arts & Community': [
    ...
    'museum','nightclub','social_club','events_venue',
  ],
};
...
const SHOP_TO_GROUP = {
  ...
  deli:'Shops', bakery:'Eat & Drink', wine:'Eat & Drink', alcohol:'Eat & Drink',
  newsagent:'Shops', florist:'Shops', books:'Shops', music:'Shops',
  clothing:'Shops', shoes:'Shops', bicycle:'Shops', hardware:'Shops',
  gifts:'Shops', pet:'Shops', stationery:'Shops', art:'Arts & Community',
...
const GROUP_EMOJI = {'Eat & Drink':'☕','Shops':'🛍','Health & Wellbeing':'💊','Arts & Community':'🎭'};
...
out center tags;`;
  ...
  try {
    ...
    const seen = new Set();
    const places = (d.elements||[]).flatMap(el => {
    ...
    const totalOpen = places.filter(p => p.status.open === true).length;
    let html = `<div class="places-summary"><strong>${totalOpen}</strong> places currently open within 900m</div>`;
    ...
    html += `<p class="places-note">
      Open status from <a href="https://www.openstreetmap.org" target="_blank" rel="noopener">OpenStreetMap</a> — may not reflect actual hours.
      ↗ links to website · ⌖ links to Google Maps.
    </p>`;
    el.innerHTML = html;
  } catch (err) {
